...
{
    public class ImportKKKhenThuongController : BaseController
    {
        // GET: ImportKKKhenThuong
        ...
        [CheckCredential(RoleID = "IMPORT_KKKHENTHUONG")]
        public ActionResult Success()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    string cl11 = dt.Rows[0]["LUONG_KK_KHAC"].ToString();
                ...
                else if (dt.Rows.Count == 0 || dt == null)
                {
                    ...
                    return Redirect("/import-luongkk-khac");
                }
                else
                {
        ...
        public ActionResult ImportDB()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->Import luong KK Khen thuong ->Import khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString() + "-Do thang luong da chot");
                ...
                else
                {
                      for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            decimal LUONG_KK_KHAC = !string.IsNullOrWhiteSpace(dt.Rows[i]["LUONG_KK_KHAC"].ToString()) ? decimal.Parse(dt.Rows[i]["LUONG_KK_KHAC"].ToString()) : 0;
                            ...
                            var rs = new ImportExcelBLL().Update_KHENTHUONG(Nam,Thang,UserName,LUONG_KK_KHAC,NhanSuID);
                            if (rs) dem++;
                            else
                            {
                    ...
                    {
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->>Import luong KK Khen thuong->Import Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString());
                    ...
                    else if (0 < dem && dem < dt.Rows.Count)
                    {
                        ...
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->Import luong KK Khen thuong->Import  khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString() + "-Dong khong import-" + rows);
                        return Redirect("/import-luongkk-khac/doc-file");
                    }
                    else
                    {
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->Import luong KK Khen thuong->Import khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString());
