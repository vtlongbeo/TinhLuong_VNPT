...
{
    public class ImportDoanhThuNgoaiController : BaseController
    {
        // GET: ImportDoanhThuNgoai
        ...
        public ActionResult Success()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    string cl11 = dt.Rows[0]["LUONGKDTM"].ToString();
                ...
                else if (dt.Rows.Count == 0 || dt == null)
                {
                    ...
                    return Redirect("/import-doanhthungoai");
                }
                else
                {
        ...
        public ActionResult ImportDB()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->Doanh thu tu xay dung ->Import khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString() + "-Do thang luong da chot");
                ...
                else
                {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            try
                            {
                                ...
                                decimal LUONGKTP = !string.IsNullOrWhiteSpace(dt.Rows[i]["LUONGKDTM"].ToString()) ? decimal.Parse(dt.Rows[i]["LUONGKDTM"].ToString()) : 0;
                                ...
                                var rs = new ImportExcelBLL().Update_DoanhThuNgoai(Nam, Thang, UserName, LUONGKTP, NhanSuID,Session[SessionCommon.DonViID].ToString());
                                if (rs) dem++;
                                else
                                {
                    ...
                    {
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->Doanh thu tu xay dung->Import Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString());
                    ...
                    else if (0 < dem && dem < dt.Rows.Count)
                    {
                        ...
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->Doanh thu tu xay dung->Import  khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString() + "-Dong khong import-" + rows);
                        return Redirect("/import-doanhthungoai/doc-file");
                    }
                    else
                    {
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->Doanh thu tu xay dung->Import khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString());
