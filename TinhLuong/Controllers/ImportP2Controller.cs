...
{
    public class ImportP2Controller : BaseController
    {
        // GET: Import hệ số P2
        SaveLog sv = new SaveLog();
        [CheckCredential(RoleID = "IMPORT_EXCEL_KDTM")]
        public ActionResult Index()
        {
           // sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->He So P2");
            return View();
        }
        ...
        public ActionResult Success()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    string cl11 = dt.Rows[0]["MYTV"].ToString();
            ...
            {
                setAlert("Cấu trúc tệp không chính xác. Vui lòng chọn lại", "error");
                return Redirect("/import-hsp2");
            }
        ...
        public ActionResult ImportDB()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->He So P2->Import khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString() + "-Do thang luong da chot");
                ...
                else
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            bl.MYTV = !string.IsNullOrWhiteSpace(dt.Rows[i]["MYTV"].ToString()) ? int.Parse(dt.Rows[i]["MYTV"].ToString()) : 0;
                            ...
                            var rs = new ImportExcelBLL().UpdateKDTM(bl);
                            if (rs) dem++;
                            else
                            {
                    ...
                    {
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->He So P2->Import Thanh Cong");
                    ...
                    else if (0 < dem && dem < dt.Rows.Count)
                    {
                        ...
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->He so P2->Import khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString() + "-Do khong import-"+rows);
                        return Redirect("/import-hsp2/doc-file");
                    }
                    else
                    {
                        sv.save(Session[SessionCommon.Username].ToString(), "Cap Nhat tu file->He so P2->Import khong Thanh Cong- Thang-" + dt.Rows[0]["Thang"].ToString() + "-nam-" + dt.Rows[0]["Nam"].ToString());
