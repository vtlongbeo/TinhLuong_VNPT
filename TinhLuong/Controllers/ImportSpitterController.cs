...
{
    public class ImportSpitterController : BaseController
    {
        ...
        [CheckCredential(RoleID = "IMPORT_EXCEL_SPITTER")]
        public ActionResult Success()
        {
            try
            {
                ...
                else if (dt.Rows.Count == 0 || dt == null)
                {
                    ...
                    return Redirect("/import-ld-splitter");
                }
                else
                {
                ...
                if (validFileTypes.Contains(extension))
                {
                    ...
                    if (extension == ".csv")
                    {
                        ...
                        if (dtPHLD.Columns.Count == 0)
                        {
                            ...
                            DataColumn c4 = new DataColumn("Spitter1", typeof(int));
                            DataColumn c5 = new DataColumn("Spitter2", typeof(int));
                            dtPHLD.Columns.Add(c0);
                            dtPHLD.Columns.Add(c1);
                            dtPHLD.Columns.Add(c2);
                            dtPHLD.Columns.Add(c3);
                            dtPHLD.Columns.Add(c4);
                            dtPHLD.Columns.Add(c5);
                        }
                    ...
                    return Redirect("/import-ld-splitter/doc-file");
                }
                else
                {
        ...
        public ActionResult ImportDB()
        {
            ...
            if (dtPH.Rows.Count > 0)
            {
                ...
                {
                    new ImportExcelBLL().Delete_KKLD(int.Parse(dtPH.Rows[0]["Nam"].ToString()), int.Parse(dtPH.Rows[0]["Thang"].ToString()), Session[SessionCommon.Username].ToString(), 3);
                    for (int i = 0; i < dtPH.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            bl.LUONGPHLD = 0;
                            bl.LUONGKKLD = 0;
                            bl.LUONGSPITTER = !string.IsNullOrWhiteSpace(dtPH.Rows[i]["Luong"].ToString()) ? decimal.Parse(dtPH.Rows[i]["Luong"].ToString()) : 0;
